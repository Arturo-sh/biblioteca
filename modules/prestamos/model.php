...
if ($_SESSION['rol_usuario'] == "Admin") {
    ...
    if (isset($_POST['edit_id'])) {
        ...
        $query_transaction_data = "SELECT t.id_transaccion, a.nombre_alumno, u.nombre_usuario, t.fecha_prestamo, t.fecha_entrega, t.estado_prestamo, l.titulo_libro FROM transaccion_prestamo AS t INNER JOIN prestamos AS p ON t.id_transaccion = p.id_transaccion INNER JOIN alumnos AS a ON t.id_alumno = a.id_alumno INNER JOIN usuarios AS u ON t.id_usuario = u.id_usuario  INNER JOIN libros AS l ON p.id_libro = l.id_libro WHERE t.id_transaccion = $id_transaccion";
        $transaction_data = mysqli_query($conn, $query_transaction_data);
        $row = mysqli_fetch_all($transaction_data, MYSQLI_ASSOC);

        $query_books_data = "SELECT l.titulo_libro FROM prestamos AS p INNER JOIN libros AS l ON p.id_libro = l.id_libro WHERE id_transaccion = $id_transaccion";
